...
def evaluate_recommendations(model_recommendations_df, actual_recommendations_df, df):
    ...
    for hotel in df.index:
        if hotel in model_recommendations_df.index and hotel in actual_recommendations_df.index:
...
def print_hotel_metrics(hotel_name, model_recommendations_df, actual_recommendations_df):
    """Compare recommendations for a specific hotel"""
    if hotel_name in model_recommendations_df.index and hotel_name in actual_recommendations_df.index:
        ...
        print(f"Model Recommendations: {list(model_recs)}")
        print(f"Actual Recommendations: {list(actual_recs)}")
...
def main():
    ...
    for hotel in df.index:
        model_recommendations[hotel] = recommendations(hotel, cosine_similarity, df, indices)
    ...
    actual_recommendations_df = pd.read_csv('Seattle_Hotels_Recommendations.csv', index_col=0)

    print("\nEvaluating recommendations...")
    metrics = evaluate_recommendations(model_rec_df, actual_recommendations_df, df)

    sample_hotels = ['The Loyal Inn', 'Shafer Baillie Mansion Bed & Breakfast', "Mildred's Bed and Breakfast"]
